...
for epoch in range(num_epochs):
    ...
    for key_idx in range(len(image_train_keys)):
        ...
        for i in range(0, images.shape[2]):
            ...
            boxes = prepare_targrets(mask)

            if len(boxes) == 0:
                continue # no box => no segmentation
            boxes_resize = torch.tensor(boxes, dtype=torch.float32) * torch.tensor([W, H, W, H], dtype=torch.float32)
            batch = {}
            ...
            with torch.set_grad_enabled(True):
                ...
                loss = dice_ce_loss(logits_up, masks_t.float()) + 0.1 * hausdorff_loss(torch.sigmoid(logits_up), masks_t.float())
                ...
                iou_train += evaluation_functions.iou(probabilities, mask, threshold=0.9)
                dice_train += evaluation_functions.dice(probabilities, mask, threshold=0.9)
                tn, fp, fn, tp = evaluation_functions.confusion_matrix_calculation(probabilities, mask, threshold=0.9)
                tn_train+= tn
                fp_train+= fp
                fn_train+= fn
                tp_train+= tp
    ...
    np.savetxt("Meas/iou_arr_train_0.txt", iou_arr_train)
    np.savetxt("Meas/dice_arr_train_0.txt", dice_arr_train)
    np.savetxt("Meas/confusion_matrix_arr_train_0.txt", confusion_matrix_arr_train)
    np.savetxt("Meas/recall_arr_train_0.txt", recall_arr_train)
    np.savetxt("Meas/precision_arr_train_0.txt", precision_arr_train)
    np.savetxt("Meas/loss_arr_train_0.txt", loss_arr_train)
    ...
    for key_idx in range(len(image_val_keys)):
        ...
        for i in range(0, images.shape[2]):
            ...
            with torch.no_grad():
                ...
                iou_val += evaluation_functions.iou(probabilities, mask, threshold=0.9)
                dice_val += evaluation_functions.dice(probabilities, mask, threshold=0.9)
                tn, fp, fn, tp = evaluation_functions.confusion_matrix_calculation(probabilities, mask, threshold=0.9)
                tn_val+= tn
                fp_val+= fp
                fn_val+= fn
    ...
    torch.save(medsam_model.state_dict(), f"Checkpoints/medsam_weights_0_{int(epoch + 1)}.pth")
    ...
    np.savetxt("Meas/iou_arr_val_0.txt", iou_arr_val)
    np.savetxt("Meas/dice_arr_val_0.txt", dice_arr_val)
    np.savetxt("Meas/confusion_matrix_arr_val_0.txt", confusion_matrix_arr_val)
    np.savetxt("Meas/recall_arr_val_0.txt", recall_arr_val)
    np.savetxt("Meas/precision_arr_val_0.txt", precision_arr_val)
    np.savetxt("Meas/loss_arr_val_0.txt", loss_arr_val)
